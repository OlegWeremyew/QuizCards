...
export const authAndProfileApi = {
  ...
  register(email: string, password: string) {
    return instance.post<RegistrationResponseType>(endPointAuthRegister, {
      email,
      password,
    });
  },
  login(email: string, password: string, rememberMe: boolean = false) {
    return instance.post<UserResponseType>(endPointAuthLogin, {
      email,
      password,
      rememberMe,
    });
  ...
  newPassword(password: string, resetPasswordToken: Undetectable<string>) {
    return instance.post<any>(endPointAuthSet_New_Password, {
      password,
      resetPasswordToken,
    });
  },
  updateProfile(data: updateProfileRequestType) {
    return instance.put<
      updateProfileRequestType,
      AxiosResponse<updateProfileResponseType, updateProfileRequestType>,
      updateProfileRequestType
    >(endPointAuthMe, data);
  },
};
